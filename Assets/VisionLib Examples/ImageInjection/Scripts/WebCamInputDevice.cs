...
namespace Visometry.VisionLib.SDK.Examples
{
    ...
    public class WebCamInputDevice : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            if (devices.Length == 0)
            {
                NotificationHelper.SendError(
                    "No camera detected",
                    "The WebCamInputDevice requires a connected camera to be able to acquire camera images.",
                    this);
                return;
            }
        ...
        public void Update()
        {
            if (!TrackingManager.DoesTrackerExistAndIsInitialized() || !this.cameraImage)
            {
                return;
            }
